...
app.get('/auth/session', requireAppAuth, (req, res) => {
...
// Uses either a service account JSON key (preferred for server/service usage)
// or Google Application Default Credentials when no key is supplied.
  ...
  try {
    ...
    if (error.code === 'ERR_GCLOUD_NOT_LOGGED_IN' || (error.message && error.message.includes('Could not load the default credentials'))) {
      res.status(401).json({
        ...
        message: 'Credentials not found or invalid. Set GOOGLE_APPLICATION_CREDENTIALS to your service-account JSON file path, or run "gcloud auth application-default login" and try again.',
      });
    } else {
...
app.post('/api-proxy', async (req, res) => {
  ...
  try {
    ...
    const apiFetchOptions = {
      method: method || 'POST',
      // Preserve server-side auth headers even if forwarded client headers contain auth keys.
